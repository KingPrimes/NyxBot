...
@Controller
@RequestMapping("/api")
public class SyndicateMissionsImageController {

    @LogInfo(title = "Api", codes = Codes.WARFRAME_SYNDICATE_SOLARIS_UNITED, businessType = BusinessType.IMAGE)
    @PostMapping(value = "/postSyndicateMissionsImage", produces = MediaType.IMAGE_PNG_VALUE)
    @ResponseBody
    public void getImage(HttpServletResponse response, @RequestBody OneBotLogInfoData data) throws IOException, HtmlToImageException {
        ...
        response.getOutputStream().write(
                HtmlToImage.conversePost(
                        Constants.LOCALHOST + "private/postSyndicateMissions",
                        data.getData()
                ).toByteArray()
        );
    }
