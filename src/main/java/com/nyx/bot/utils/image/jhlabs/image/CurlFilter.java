        ...
        final private int getPixel(int[] pixels, int x, int y, int width, int height) {
            if (x < 0 || x >= width || y < 0 || y >= height) {
                return switch (edgeAction) {
                    default -> 0;
                    case WRAP -> pixels[(ImageMath.mod(y, height) * width) + ImageMath.mod(x, width)];
                    case CLAMP ->
                            pixels[(ImageMath.clamp(y, 0, height - 1) * width) + ImageMath.clamp(x, 0, width - 1)];
                };
            }
    ...
    final private int getPixel(int[] pixels, int x, int y, int width, int height) {
        if (x < 0 || x >= width || y < 0 || y >= height) {
            return switch (edgeAction) {
                default -> 0;
                case WRAP -> pixels[(ImageMath.mod(y, height) * width) + ImageMath.mod(x, width)];
                case CLAMP -> pixels[(ImageMath.clamp(y, 0, height - 1) * width) + ImageMath.clamp(x, 0, width - 1)];
            };
        }
        return pixels[y * width + x];
    }
