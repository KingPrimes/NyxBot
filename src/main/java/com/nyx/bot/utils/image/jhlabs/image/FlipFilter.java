...
public class FlipFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        int newH = h;
        switch (operation) {
            case FLIP_H -> newX = width - (x + w);
            case FLIP_V -> newY = height - (y + h);
            case FLIP_HV -> {
                newW = h;
                newH = w;
                newX = y;
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                int newCol = col;
                switch (operation) {
                    case FLIP_H -> newCol = w - col - 1;
                    case FLIP_V -> newRow = h - row - 1;
                    case FLIP_HV -> {
                        newRow = col;
                        newCol = row;
                    }
    ...
    public String toString() {
        return switch (operation) {
            case FLIP_H -> "Flip Horizontal";
            case FLIP_V -> "Flip Vertical";
            case FLIP_HV -> "Flip Diagonal";
            case FLIP_90CW -> "Rotate 90";
            case FLIP_90CCW -> "Rotate -90";
            case FLIP_180 -> "Rotate 180";
            default -> "Flip";
        };
    }
}
