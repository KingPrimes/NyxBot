...
public class FBMFilter extends PointFilter implements Cloneable {
	...
	public void setBasisType(int basisType) {
		...
		switch (basisType) {
			default -> basis = new Noise();
			case RIDGED -> basis = new RidgedFBM();
			case VLNOISE -> basis = new VLNoise();
			case SCNOISE -> basis = new SCNoise();
			case CELLULAR -> basis = new CellularFunction2D();
		}
	}
