...
public class FissuresImageController {
    @LogInfo(title = "Api", codes = Codes.WARFRAME_ACTIVE_MISSION_PLUGIN, businessType = BusinessType.IMAGE)
    ...
    public void postFissuresImage(HttpServletResponse response, @RequestBody OneBotLogInfoData data) throws IOException, HtmlToImageException {
        ...
        switch (data.getCodes()) {
            case WARFRAME_VOID_STORMS_PLUGIN -> i = 1;
            case WARFRAME_ACTIVE_MISSION_PATH_PLUGIN -> i = 2;
            default -> i = 0;
        }
    ...
    @LogInfo(title = "Api订阅", codes = Codes.WARFRAME_ACTIVE_MISSION_PLUGIN, businessType = BusinessType.IMAGE)
