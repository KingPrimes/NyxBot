...
public class RivenAnalyseTrendService {
    ...
    private List<RivenAnalyseTrend> getRivenAnalyseTrends() {
        ...
        for (String url : ApiUrl.WARFRAME_DATA_SOURCE_RIVEN_ANALYSE_TREND) {
            HttpUtils.Body body = HttpUtils.sendGet(url);
            if (body.code().is2xxSuccessful()) {
                try {
                    rivenAnalyseTrends.addAll(objectMapper.readValue(body.body(), new TypeReference<List<RivenAnalyseTrend>>() {
                    }));
                    break;
                } catch (Exception e) {
                    // 忽略解析错误，继续处理其他数据源
                    log.warn("解析RivenAnalyseTrends数据失败，尝试下一个数据源: {}", e.getMessage());
                }
            } else {
                log.warn("获取RivenAnalyseTrends数据失败，尝试下一个数据源: HttpCode {} - Url:{}", body.code(), url);
            }
        }
        return rivenAnalyseTrends;
    }
