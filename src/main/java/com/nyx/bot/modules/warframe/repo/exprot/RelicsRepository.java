...
public interface RelicsRepository extends JpaRepository<Relics, String>, JpaSpecificationExecutor<Relics> {
    ...
    List<Relics> findByRelicRewardsRewardNameContainingIgnoreCase(String rewardName);
     ...
     * @return Page<Relics>
     */
    @Query("select r from Relics r where (:#{#re.name} is null or LOWER(r.name) = LOWER(:#{#re.name}))")
     ...
     * @param name 遗物名称
     */
    List<Relics> findByNameIgnoreCase(String name);
     ...
     * @param name 遗物名称
     */
    List<Relics> findByNameContainingIgnoreCase(String name);
}
